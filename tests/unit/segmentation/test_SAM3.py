...
class TestSAM3(unittest.TestCase):
    ...
    def test_dry_run(self) -> None:
        """Test dry run mode."""
        result = run_processing(dataset_name=self.dataset_name, dry_run=True, verbose=True)
    ...
    def test_run_processing_grouped(self) -> None:
        ...
        run_processing(dataset_name=self.dataset_name, prompt="jaguar", threshold=0.6, device="cpu", verbose=True)  # Avoid torch import check
    ...
    def test_run_processing_flat(self) -> None:
        ...
        run_processing(dataset_name=self.dataset_name, device="cpu", verbose=True)
    ...
    def test_summary_generation(self) -> None:
        ...
        with patch("builtins.open", new_callable=MagicMock) as mock_open, patch("json.dump") as mock_json_dump:
            run_processing(dataset_name=self.dataset_name, summary_location=Path("test_summary.json"), device="cpu")
