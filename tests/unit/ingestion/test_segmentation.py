...
from jaguars.ingestion.processing.segmentation import (
    filter_by_count,
    filter_unexpected_segmentations,
    cleanup_dataset,
    run_processing
)
...
class TestSegmentationProcessing(unittest.TestCase):
    ...
    def test_filter_by_count(self) -> None:
        ...
        s1[self.segmentation_field] = fo.Detections(
            detections=[fo.Detection(label="jaguar", confidence=0.9)]
        )
        samples.append(s1)
        ...
        s2[self.segmentation_field] = fo.Detections(
            detections=[
                fo.Detection(label="jaguar", confidence=0.8),
                fo.Detection(label="jaguar", confidence=0.7)
            ]
        )
        samples.append(s2)
    ...
    def test_filter_unexpected_segmentations(self) -> None:
        ...
        s_good[self.segmentation_field] = fo.Detections(
            detections=[fo.Detection(
                label="jaguar", 
                confidence=0.9, 
                bounding_box=[0.2, 0.2, 0.5, 0.5] # Area = 0.25 (Good)
            )]
        )
        ...
        s_small[self.segmentation_field] = fo.Detections(
            detections=[fo.Detection(
                label="jaguar", 
                confidence=0.9, 
                bounding_box=[0.1, 0.1, 0.05, 0.05] # Area = 0.0025 < 0.01
            )]
        )
        ...
        count = filter_unexpected_segmentations(
            self.dataset, 
            self.segmentation_field,
            min_confidence=0.5,
            min_area_rel=0.01,
            max_area_rel=0.9
        )
    ...
    def test_run_processing(self, mock_sam3: MagicMock) -> None:
        ...
        s1[self.segmentation_field] = fo.Detections(
            detections=[fo.Detection(label="jaguar", confidence=0.9, bounding_box=[0,0,0.5,0.5])]
        )
        ...
        result_ds = run_processing(
            dataset_name=self.dataset_name,
            segmentation_field=self.segmentation_field,
            dry_run=False
        )
