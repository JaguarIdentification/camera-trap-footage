...
def run_processing(
    ...
    """Core Logic for video frame sampling.

    Samples frames from videos in the dataset using a two-phase strategy:
    ...
    Sampled frames are added to the same group as their source video, in the
    "image" slice.

    Args:
        dataset_name: Name of the FiftyOne grouped dataset containing videos
        ...
        late_fps: Frames per second to sample for remainder (default: ~0.167 = 1 frame per 6s)
        output_format: Image format for sampled frames (jpg, png, etc.)
        summary_location: Path to write summary JSON
        dry_run: If True, only logs what would be done
        verbose: Enable detailed logging

    Returns:
        Updated FiftyOne dataset with sampled frames added, or None if dry_run
    """
    logger_instance = setup_logger(MODULE_NAME, level=logging.DEBUG if verbose else logging.INFO)
    logger_instance.info("Starting video frame sampling for dataset: %s", dataset_name)
    logger_instance.info("Sampling strategy: %s fps for first %s seconds, then %s fps for remainder", early_fps, early_seconds, late_fps)

    validate_resources(dataset_name)

    if dry_run:
        logger_instance.info("DRY RUN: Would sample frames from videos in dataset '%s'", dataset_name)
        return None

    # Load dataset and sample frames
    ...
    if summary_location:
        ...
        image_count = len(dataset.select_group_slices(DEFAULT_GROUP_SLICE))
        video_count = len(dataset.select_group_slices("video"))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Frames per second to sample in first early_seconds (default: %(default)s)",
    )
    parser.add_argument(
        "--early-seconds",
        type=float,
        default=5.0,
        help="Duration in seconds to sample at early_fps (default: %(default)s)",
    )
    parser.add_argument(
        "--late-fps",
        type=float,
        default=1.0 / 6.0,
        help="Frames per second to sample for remainder (default: %(default)s = 1 frame per 6s)",
    )
    parser.add_argument(
        "--output-format",
        type=str,
        default="jpg",
        help="Image format for frames (default: %(default)s)",
    )
    parser.add_argument(
        "--summary-location",
        type=Path,
    ...
    try:
        ...
        logger.error("Error during frame sampling: %s", e, exc_info=True)
        raise
