...
Also force validation failure and assert neither final nor owned staging dataset remains. Add adversarial real-database cases for generated staging-name collisions, constructor races, rename exceptions both before and after persistence, and old-backup deletion errors both before and after their database effect. Assert cleanup occurs only when the generated dataset ID and unguessable metadata token prove ownership; an unproven post-metadata constructor artifact must be retained and reported rather than deleted. After verified promotion, backup cleanup failures must retain the valid new final and report the re-queried backup state.
